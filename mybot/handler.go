package mybot

import (
	"database/sql"
	"log"

	tgbotapi "github.com/go-telegram-bot-api/telegram-bot-api/v5"
)

// HandleMessage - обрабатывает сообщение
func HandleMessage(bot *tgbotapi.BotAPI, msg *tgbotapi.Message,
	forwardChatID int64, db *sql.DB, botUsername string) {

	log.Printf("💬 Сообщение от @%s: %s", msg.From.UserName, msg.Text)

	// ===============================================
	// ПРОВЕРКА: Не обрабатываем команды как ввод паттерна
	// ===============================================
	if msg.IsCommand() {
		handleCommand(bot, msg, db)
		return
	}

	// ===============================================
	// 1. ПРОВЕРКА ВВОДА ПАТТЕРНА (новое!)
	// ===============================================
	if ProcessPatternInput(bot, msg, db) {
		log.Printf("✅ Обработан ввод паттерна от @%s", msg.From.UserName)
		return // Не обрабатываем дальше
	}

	// ===============================================
	// 2. ПРОВЕРКА ВВОДА ОТВЕТА (новое!)
	// ===============================================
	if ProcessResponseInput(bot, msg, db) {
		log.Printf("✅ Обработан ввод ответа от @%s", msg.From.UserName)
		return // Не обрабатываем дальше
	}

	// ===============================================
	// 3. ПРОВЕРКА ВВОДА НОВОГО ТРИГГЕРА (новое!)
	// ===============================================
	if ProcessNewTriggerInput(bot, msg, db) {
		log.Printf("✅ Обработан ввод нового триггера от @%s", msg.From.UserName)
		return // Не обрабатываем дальше
	}

	// ===============================================
	// 4. Сохраняем в БД (быстро, для логов)
	// ===============================================
	if db != nil {
		SaveMessageToDB(db, botUsername, msg)
	}

	// ===============================================
	// 5. ДЕТАЛЬНОЕ сохранение (новое!)
	// ===============================================
	if db != nil {
		go func() {
			if err := SaveMessageDetailed(db, &bot.Self, msg); err != nil {
				log.Printf("⚠️ Ошибка детального сохранения: %v", err)
				// НЕ прерываем работу бота!
			}
		}()
	}

	// ===============================================
	// 6. Пересылаем оригинальное сообщение
	// ===============================================
	forwardMessage(bot, msg, forwardChatID)

	// ===============================================
	// 7. Отправляем лог в отдельный чат
	// ===============================================
	SendMessageLog(bot, msg, botUsername, bot.Self.ID)

	// ID чата для логов
	logChatID := int64(-1003655803421)

	// ===============================================
	// 8. ПРОВЕРКА ВСЕХ ТРИГГЕРОВ
	// ===============================================
	if CheckAllTriggers(bot, msg, logChatID, db) {
		return
	}
}

// HandleCallback - обрабатывает callback-запросы от inline-кнопок
// ТОЛЬКО ВЫЗОВ функции из callbacks.go - логика там!
func HandleCallback(bot *tgbotapi.BotAPI, callbackQuery *tgbotapi.CallbackQuery, db *sql.DB) {
	log.Printf("🔄 [Handler] Callback от @%s", callbackQuery.From.UserName)

	// ===============================================
	// ВЫЗОВ ФУНКЦИИ ИЗ ОТДЕЛЬНОГО МОДУЛЯ
	// ===============================================
	HandleCallbackQuery(bot, callbackQuery, db)
}
