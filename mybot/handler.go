...
func HandleMessage(bot *tgbotapi.BotAPI, msg *tgbotapi.Message,
	...
	// ===============================================
	// 1. Сохраняем в БД (быстро, для логов)
	// ===============================================
	if db != nil {
	...
	// ===============================================
	// 2. ДЕТАЛЬНОЕ сохранение (новое!)
	// ===============================================
	if db != nil {
		go func() {
			if err := SaveMessageDetailed(db, &bot.Self, msg); err != nil {
				log.Printf("⚠️ Ошибка детального сохранения: %v", err)
	...
	// ===============================================
	// 3. Пересылаем оригинальное сообщение
	// ===============================================
	forwardMessage(bot, msg, forwardChatID)

	// ===============================================
	// 4. Отправляем лог в отдельный чат
	// ===============================================
